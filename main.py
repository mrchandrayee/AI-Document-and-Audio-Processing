...
def chatCompletion(file: Annotated[UploadFile, File()], prompt: Annotated[str, Form()]):
    documentText = ''
    b64 = None
    ...
    if b64 is not None:
        userContent.append({
            "type": "image_url",
            "image_url": {"url": f"https://i.ibb.co/rxBnBqQ/image1-1.jpg"}
        })
    ...
    # pdf.add_section(Section(response.choices[0].message.content))
    # pdf.save(f"response.pdf")
    # response = FileResponse(f"response.pdf",
